...
export const socket = io(SOCKET_URL, { autoConnect:false, reconnection:true, reconnectionAttempts:5, reconnectionDelay:1000 });
...
export const useSocket = () => {
  ...
  const connect = username => { socket.connect(); if(username) socket.emit('user_join',username); };
  ...
  useEffect(()=>{
    ...
    const onReceiveMessage = message => { setLastMessage(message); setMessages(prev => [...prev,message]); };
    const onPrivateMessage = message => { setLastMessage(message); setMessages(prev => [...prev,message]); };
